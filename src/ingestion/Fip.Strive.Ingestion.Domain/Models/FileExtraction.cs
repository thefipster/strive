...
using Fip.Strive.Core.Ingestion.Domain.Extensions;
...
public class FileExtraction
{
    ...
    public FileExtraction(DataSources source, string sourceFile, DateTime timestamp, DataKind kind)
        : this()
    {
        Source = source;
    ...
    public byte[] ToHash()
    {
        var metrics = string.Join(",", Attributes.OrderBy(x => x.Key).Select(x => x.ToString()));
        ...
        var events = string.Join(",", Events.OrderBy(x => x.Timestamp).Select(x => x.ToString()));
