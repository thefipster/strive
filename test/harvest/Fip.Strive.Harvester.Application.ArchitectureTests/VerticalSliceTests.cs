...
public class VerticalSliceTests
{
    ...
    private const string InfrastructureNamespace = "Fip.Strive.Harvester.Application.Infrastructure";

    [Fact]
    public void Features_Should_Not_Depend_On_Each_Other()
    {
        // Dynamically discover all feature namespaces
        var allFeatureNamespaces = Architecture.Namespaces
            .Where(ns => ns.FullName.StartsWith(FeaturesNamespace) && 
        ...
        foreach (var featureNamespace in allFeatureNamespaces)
        {
            ...
            {
                var rule = Types()
                    .That().ResideInNamespace(featureNamespace)
                    .Should().NotDependOnAny(
                        otherFeatures.Aggregate(
                                Types().That().ResideInNamespace(otherFeatures.First()),
                                (current, ns) => current.Or().ResideInNamespace(ns))
                            .As($"Other features (excluding {featureNamespace})"))
                    .Because($"feature {featureNamespace} should be independent and not reference other features");
        ...
        Assert.True(allFeatureNamespaces.Count > 0, 
    ...
    public void Core_Should_Not_Depend_On_Features()
    {
        ...
        var allCoreNamespaces = Architecture.Namespaces
            .Where(ns => ns.FullName.StartsWith(CoreNamespace) && 
                         ns.FullName.Length > CoreNamespace.Length)
